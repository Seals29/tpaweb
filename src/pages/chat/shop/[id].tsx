...
export default function main(props: any) {
    ...
    useEffect(() => {
        ...
        const newSocket = new WebSocket("ws://localhost:9998/message", "1");
        // headers: {
    ...
    const handleSend = () => {
        if (socket.readyState === WebSocket.OPEN) {
            const obj = { from: "1", to: "2", content: content }
        ...
        <div className={style.chatContainer}>
            <div className={style.chatmessages} style={{ color: 'black' }}>
                {messages.map((message: any, index) => (
                    ...
                    >
                        <div>From : {message.from === "1" ? "1" : "2"}</div>
                        <div>To : {message.to === "2" ? "2" : "1"}</div>
...
export async function getStaticProps(context: any) {
    ...
    const res = await axios.get(`http://localhost:9998/getusershopid/${id}`)
    const toUser = await res.data;
