...
export default function ProductDetail(props: any) {
  ...
  const addToCart = (e: any) => {
    ...
    if (currCount > 0) {
      ...
      axios
        .post("http://localhost:9998/insertcart", data)
        .then((res) => {
          console.log(res);
        })
        .catch((err) => {
  ...
  const saveToWishList = (e: any) => {
    ...
    console.log(data);
    axios
      .post("http://localhost:9998/AddNewProductIntoWishList", data)
      .then((res) => {
        console.log(res);
        if (res.data.error) {
      ...
      <div className={blur === true ? style.maindiv : ""}>
        ...
        <div
          style={{
            ...
            padding: "10px",
            textAlign: "center",
            display: `${shop.isban ? "" : "none"}`,
          }}
        >
          Current Product is banned!
        </div>
        <div
          style={{
            backgroundColor: "red",
          ...
          >
            ...
            >
              ...
              <div style={{ color: theme.text }}>
                <div
                  className={style.productdetailinfo}
                ...
                <div>
                  <div className={style.detailstock}>
                    {product.stock ? product.stock : "No"}{" "}
                    {Lang.isEng ? "Stock Left" : "Stok tersisa"}
                    <br />
                ...
                <br />
                <div className={style.detailcategory}>
                  {Lang.isEng ? "Category" : "Kategori"} : {product.category}
                </div>
                <br />
                <div className={style.detailcategory}>
                ...
                >
                  ...
                  <button onClick={addToCart}>
                    ...
                    {Lang.isEng ? " Add to Cart" : " Tambahkan ke keranjang"}
                  </button>
                </div>
              </div>
            </div>
...
export async function getStaticProps(context: any) {
  ...
  const res2 = await axios.get(
    `http://localhost:9998/getoneshop/${product.shopid}`
  );
  ...
  const res5 = await axios.get(
    `http://localhost:9998/getsimilarproductbycategory?category=${product.category}`
  );
