        ...
        <div>
            ...
            <div className={style.bannerbtn}>
                <button onClick={((e: any) => {
                    setIsActive(true)
                })}><FontAwesomeIcon icon={faPlus} /> New Banner Promotion</button>
            </div>
            {allbanners.map(banner => (
                <div className={style.bannercontainer}>
                    <div><FontAwesomeIcon icon={faClose} style={{
                        display: 'flex', flexDirection: 'column', alignContent: 'flex-end', float: 'right'
                    }} onClick={((item: any) => {
                        item.preventDefault()
                        const data = {
                        ...
                        axios.post("http://localhost:9998/rmvpromotionbanner", data).then(res => {
                            console.log(res);
            ...
            <div className={style.centerContainer} style={{ backgroundColor: theme.background2, display: isActive ? "" : "none" }}>
                <FontAwesomeIcon icon={faClose} style={{
                    top: '0%', float: 'right', right: '0', paddingLeft: '80%', color: theme.text, cursor: 'pointer'
                ...
                <button style={{ backgroundColor: theme.button, color: theme.text }}
                    onClick={(e: any) => {
                        const imageRef = ref(storage, `promotion/${imageUpload.name + "OldEgg"}`);
                            ...
                            res.items.forEach((item) => {
                                if (item.name === `${imageUpload.name + "OldEgg"}`) {
                                    getDownloadURL(item).then((urls: any) => {
                                        ...
                                        axios.post("http://localhost:9998/addpromotionbanner", data).then(res => {
                                            console.log(res)
                                        }).catch(err => {
                                            console.log(err)
                                        })
